...
- **Auto3dLabel**：调研 ✅，MVP 路线定案（2D → 3D 提升）；**v0.1 单帧 MVP ✅ 完成（2026-08-24）**——五步管线（标定 → 反投影 → 聚类拟合 → 导出/评测）+ Agentic 闭环 + Web 复核，质量门全绿（见 `auto3dlabel/milestone/v0.1.md`、实测 `auto3dlabel/tests/test-v0.1.md`、速查 `auto3dlabel/CLAUDE.md`），KITTI object 数据已就位（`Documents/datasets/KITTI/object`）
