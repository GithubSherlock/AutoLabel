...
def run_segmentation(
    ...
    batch_size, num_workers = resolve_batch_params(
        "object_detection", infer_fn, sample_image_paths(gt, img_dir),
...
def run_detection_only(
    ...
    分块批量推理（detect_batch_or_fallback + OOM 降级逐图）。
    """
    try:
        from tqdm import tqdm
        has_tqdm = True
    ...
    iterator = tqdm(range(0, total, batch_size), total=n_batches,
                    desc=f"检测（{det_model_name} b={batch_size}）", unit="batch") \
        if has_tqdm else range(0, total, batch_size)
...
def run_self_detect_segmentation(
    ...
    generate_batch 整块推理（bboxes 传占位 []）；无批量能力（语义分割等）
    或批量 OOM 时自动降级逐图（generate_batch_or_fallback）。
    """
    try:
        from tqdm import tqdm
        has_tqdm = True
    ...
    # 批量推理超参：CLI 显式 > 动态实测（模型加载后测单图显存）> 静态表
    _seg_batch = getattr(seg_model, "generate_batch", None)
    infer_fn = (
        (lambda ps: _seg_batch(ps, [], mode="box"))
        if _seg_batch is not None else None
    )
    batch_size, num_workers = resolve_batch_params(
        "instance_segmentation", infer_fn, sample_image_paths(gt, img_dir),
        explicit_batch=batch, explicit_workers=workers,
    )
    print(f"批量推理: batch_size={batch_size}  num_workers={num_workers}")
    ...
    iterator = (
        tqdm(range(0, total, batch_size), total=n_batches,
             desc=f"self-detect（{seg_model_name} b={batch_size}）", unit="batch")
        if has_tqdm else range(0, total, batch_size)
    )
    ...
    for start in iterator:
        ...
        masks_per_img = generate_batch_or_fallback(seg_model, chunk_paths, [])
        for img_id, masks in zip(chunk_ids, masks_per_img):
            # 读取原图尺寸
            im = _Image.open(img_dir / gt[img_id]["file_name"])
            h, w = im.height, im.width
...
def main() -> None:
    ...
    if args.det_only:
        ...
        # box-prompted：每图 GT bbox prompt 不同，保持逐图
        mode = "box-prompted"
