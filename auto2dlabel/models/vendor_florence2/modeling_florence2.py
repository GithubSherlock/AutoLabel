...
def _past_to_legacy(past_key_values, idx):
    """transformers 5.x Cache 抽象 → 官方建模旧式按层 (self_k, self_v, cross_k, cross_v)。

    5.x 起 past_key_values 是 Cache 对象（EncoderDecoderCache，无 __getitem__），
    官方建模按旧式 tuple-of-tuples 消费；decoder 循环入口转换一次，此后模型
    自行返回/消费旧式 tuple。Cache.__iter__ 按层 yield (keys, values, sliding)，
    keys/values 与旧式 (k, v) 形状一致，直接取用。
    """
    if past_key_values is None:
        return None
    if hasattr(past_key_values, "get_seq_length"):  # 5.x Cache
        sc = getattr(past_key_values, "self_attention_cache", past_key_values)
        layers = list(sc)
        if layers[idx][0] is None:  # 预分配未初始化层（首轮 prefill）→ 无 past
            return None
        k, v = layers[idx][0], layers[idx][1]
        cc = getattr(past_key_values, "cross_attention_cache", None)
        if cc is not None:
            c_layers = list(cc)
            return (k, v, c_layers[idx][0], c_layers[idx][1])
        return (k, v)
    return past_key_values[idx]  # 旧式 tuple-of-tuples


def _past_length(past_key_values):
    """past 序列长度：5.x Cache 用 get_seq_length()，旧式 tuple 用 [0][0].shape[2]。"""
    if past_key_values is None:
        return 0
...
class Florence2Decoder(Florence2LanguagePreTrainedModel):
    ...
    def forward(
        ...
        past_key_values_length = _past_length(past_key_values)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            past_key_value = _past_to_legacy(past_key_values, idx)
...
class Florence2LanguageModel(Florence2LanguagePreTrainedModel):
    _tied_weights_keys = {"decoder.embed_tokens.weight": "encoder.embed_tokens.weight"}  # 5.15 dict 格式（官方 list 不生效）
    ...
    # 官方 _tie_weights 已移除（5.15 meta-init 阶段执行会污染 meta 张量）；
    # 5.15 tie 走 _tied_weights_keys dict（见类声明处补丁）
    def get_input_embeddings(self):
        return self.shared
...
class Florence2PreTrainedModel(PreTrainedModel):
    ...
    def _supports_flash_attn_2(self):
        ...
        """
        # transformers 5.x 移除了 _supports_* 标志（统一 attention 分派）
        lm = getattr(self, "language_model", None)  # __init__ 早期未构造时为 None
        return getattr(lm, "_supports_flash_attn_2", False) if lm is not None else False
    ...
    def _supports_sdpa(self):
        ...
        """
        # transformers 5.x 移除了 _supports_* 标志（统一 attention 分派）；
        # torch 2.x 恒支持 SDPA，兼容补丁返回 True
        lm = getattr(self, "language_model", None)  # __init__ 早期未构造时为 None
        return getattr(lm, "_supports_sdpa", True) if lm is not None else True
...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel):
    ...
    def from_pretrained(cls, *args, **kwargs):
        """加载后手动共享词嵌入（5.15 兼容补丁，见 vendor README 补丁 7）。

        5.15 的 meta-init + MISSING 记账流程下，checkpoint 仅存
        `language_model.model.shared.weight` 一份，encoder/decoder
        embed_tokens 与 lm_head 的 tie 不生效（保持 meta 张量 → 生成
        必为垃圾文本）。此处把三者绑到 shared 的同一 Parameter。
        """
        model = super().from_pretrained(*args, **kwargs)
        lm = model.language_model
        shared = lm.model.shared.weight
        lm.model.encoder.embed_tokens.weight = shared
        lm.model.decoder.embed_tokens.weight = shared
        lm.lm_head.weight = shared
        return model
