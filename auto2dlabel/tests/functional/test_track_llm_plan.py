...
def test_llm_plan_parses_json(monkeypatch: pytest.MonkeyPatch) -> None:
    prompts, t = _plan_with_fake(
        '{"prompts": ["person"], "threshold": 0.3}', monkeypatch)
...
def test_llm_plan_null_threshold(monkeypatch: pytest.MonkeyPatch) -> None:
    _, t = _plan_with_fake('{"prompts": ["person"], "threshold": null}', monkeypatch)
    assert t is None
...
def test_resolve_plan_without_llm_uses_extract_prompts(monkeypatch: pytest.MonkeyPatch) -> None:
    prompts, t = resolve_plan("检测行人", False, "deepseek", None, None, None, 0.1)
    assert prompts == ["person"]
    assert t == 0.1


def test_resolve_plan_llm_success_overrides(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(cli_track, "_llm_plan_once", lambda *a, **k: (["car"], 0.5))
    prompts, t = resolve_plan("复杂指令", True, "deepseek", None, None, None, 0.1)
    assert prompts == ["car"]
    assert t == 0.5


def test_resolve_plan_llm_null_threshold_keeps_cli(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(cli_track, "_llm_plan_once", lambda *a, **k: (["car"], None))
    _, t = resolve_plan("复杂指令", True, "deepseek", None, None, None, 0.1)
    assert t == 0.1


def test_resolve_plan_llm_failure_falls_back(monkeypatch: pytest.MonkeyPatch) -> None:
    def boom(*a: object, **k: object) -> tuple[list[str], float | None]:
        raise RuntimeError("network down")

    monkeypatch.setattr(cli_track, "_llm_plan_once", boom)
    prompts, t = resolve_plan("检测行人", True, "deepseek", None, None, None, 0.1)
    assert prompts == ["person"]  # 回退 extract_prompts
    assert t == 0.1
